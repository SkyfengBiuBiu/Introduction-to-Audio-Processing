...
# Find peaks.
peak_indexes = peakutils.indexes(mag_X, thres=0.6, min_dist=np.round(100 / fs * win_len))
...
y =  A * np.sin( 2*np.pi*f_carrier*t_scale + ind_mod*np.sin(2*np.pi*f_mod*t_scale))

# Perform FFT: remember to apply a hann window to the signal before FFT (as in the first part)
Y = scipy.fft(y[t_start:t_start+win_len]*win)
...
plt.subplot(2, 1, 1)
plt.plot(t_scale, y)
plt.title('Time domain visualization of the FM synthesis')
plt.grid('on')
plt.xlabel('Time (s)')
plt.ylabel('Amplitude')

plt.subplot(2, 1, 2)
plt.plot(freq_scale, mag_Y)
plt.title('Frequency domain visualization of the FM synthesis')
plt.grid('on')
plt.ylabel('Magnitude (dB)')
plt.xlabel('Frequency (Hz)')
plt.tight_layout()
...
#audio_name = '1.wav' % (f_carrier, str(ind_mod))
wavfile.write('1.wav', fs, np.asarray(y / np.max(np.abs(y)), dtype = np.int16))

#wavfile.write('1.wav', fs,wavData)
#winsound.PlaySound('1.wav', winsound.SND_FILENAME)
winsound.PlaySound('oboe59.wav', winsound.SND_FILENAME)
